...
boroughs = gpd.read_file("data/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp").to_crs(epsg=27700)
...
```{python}
# Filtering Data again as Q6 has other filters in place
from datetime import datetime, timedelta
...
print(f"Mean Deprivation Rank for 'at-risk' wards: {averages.loc['at_risk'].iloc[0]:.0f}")
print(f"Mean Deprivation Rank for 'too-late' wards: {averages.loc['too_late'].iloc[0]:.0f}")
print(f"Mean Deprivation Rank for 'neither' wards: {averages.loc['neither'].iloc[0]:.0f}")
```
...
legend_elements = [mpatches.Patch(color=colors[i], label=legend_labels[i]) for i in range(4)]
